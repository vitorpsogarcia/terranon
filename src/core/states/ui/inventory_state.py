...
class InventoryState(BaseState):
    ...
    def draw(self, surface: pygame.Surface):
        ...
        rect = pygame.Rect(
            100, 100, self.screen_size[0] - 200, self.screen_size[1] - 200
        )
        ...
        close_hint = pygame.font.SysFont("Arial", 24).render(
            "Press ESC or I to close", True, Colors.text.secondary
        )
        surface.blit(
            inventory_text,
            (self.screen_size[0] // 2 - inventory_text.get_width() // 2, 120),
        )
        surface.blit(
            close_hint,
            (
                self.screen_size[0] // 2 - close_hint.get_width() // 2,
