...
class GameOverState(BaseState):
    ...
    def draw(self, surface):
        ...
        enter_to_continue = self.small_font.render(
            "Press ENTER to continue", True, Colors.text.primary
        )
        surface.blit(
            game_over_text,
            (
                self.screen_size[0] // 2 - game_over_text.get_width() // 2,
                self.screen_size[1] // 2 - enter_to_continue.get_height() // 2,
            ),
        )
        surface.blit(
            enter_to_continue,
            (
                self.screen_size[0] // 2 - enter_to_continue.get_width() // 2,
                self.screen_size[1] // 2 + enter_to_continue.get_height() + 30,
            ),
        )
