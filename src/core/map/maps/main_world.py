...
class MainWorldMap(Map):
    ...
    def _process_map(self):
        ...
        waypoints_layer = self.tiled_map.get_layer_by_name(MapLayersEnum.WAYPOINTS.value)
        spawners_layer = self.tiled_map.get_layer_by_name(MapLayersEnum.SPAWNERS.value)
        enemy_routes_layer = self.tiled_map.get_layer_by_name(MapLayersEnum.ENEMY_ROUTES.value)
    ...
    def _process_waypoints_layer(self, layer):
        ...
        for item in layer:
            ...
            if waypoint_name:
                waypoints[WaypointsEnum(waypoint_name)] = Waypoint(waypoint_name, pygame.math.Vector2(item.x, item.y))
